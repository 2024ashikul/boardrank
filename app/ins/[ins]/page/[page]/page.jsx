...
export default async function Page({ params }) {
  ...
  console.log(temp);
  const count = Math.ceil(temp / 100)
  ...
  return (
    <>
      <div className="flex justify-center gap-3 ">
        <div className="bg-amber-200 px-4 py-1 rounded-sm"><Link href={`/ins/${encodeURIComponent(institute)}/page/${page-1}`}>Prev</Link></div>
        {paginationList.map((page, index) => (
          <div key={index} className="bg-amber-200 px-4 py-1 rounded-sm"><Link href={`/ins/${encodeURIComponent(institute)}/page/${page}`}>{page}</Link></div>
        ))}
        <div className="bg-amber-200 px-4 py-1 rounded-sm"><Link href={`/ins/${encodeURIComponent(institute)}/page/${page+1}`}>Next</Link></div>
      </div>
            ...
            {students.map((student,i) => {
              ...
              return (
                <tr key={student.roll_no} className={`${(rank+i) % 2 == 0 ? 'bg-green-200' : 'bg-green-100'} h-12`}>
                  ...
                  <td className="px-5">{student.institute}</td>
